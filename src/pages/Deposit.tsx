...
export default function Deposit() {
  ...
  const handleCustomAmountChange = (value: string) => { const n = parseInt(value) || 0; if (n >= 0) { setCustomAmount(value); setSelectedAmount(null); } };
  const getFinalAmount = () => selectedAmount || parseInt(customAmount) || 0;
      ...
      <div className="p-4 pb-8">
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="bg-gradient-to-br from-primary/20 via-primary/10 to-transparent border border-primary/20 rounded-2xl p-5">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-primary/20 flex items-center justify-center"><Wallet className="h-6 w-6 text-primary" /></div>
                <div><p className="text-sm text-muted-foreground">Current Balance</p><p className="text-2xl font-bold text-foreground">₹{balance.toFixed(2)}</p></div>
              </div>
            </div>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {PRESET_AMOUNTS.map((item) => (
                  <button key={item.amount} onClick={() => handleAmountSelect(item.amount)}
                    className={`relative h-20 rounded-xl border-2 transition-all duration-200 flex flex-col items-center justify-center gap-1 ${selectedAmount === item.amount ? "border-primary bg-primary/10 shadow-lg shadow-primary/20" : "border-border bg-card hover:border-primary/50 hover:bg-card/80"}`}>
                    {item.popular && <span className="absolute -top-2 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-primary text-primary-foreground text-[10px] font-semibold rounded-full flex items-center gap-1"><Sparkles className="h-3 w-3" /> Popular</span>}
                    <span className={`text-2xl font-bold ${selectedAmount === item.amount ? "text-primary" : "text-foreground"}`}>{item.label}</span>
                  </button>
                ))}
              </div>
            </div>
            <div className="relative"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div><div className="relative flex justify-center"><span className="bg-background px-3 text-xs text-muted-foreground uppercase tracking-wider">or enter custom</span></div></div>
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-lg bg-muted flex items-center justify-center"><IndianRupee className="h-5 w-5 text-muted-foreground" /></div>
              <Input type="number" placeholder="Enter amount" value={customAmount} onChange={(e) => handleCustomAmountChange(e.target.value)} className="pl-16 h-14 text-xl font-semibold bg-card border-border rounded-xl focus:border-primary focus:ring-primary" />
            </div>
            <Button className="w-full h-14 text-lg font-semibold rounded-xl bg-primary hover:bg-primary/90 shadow-lg shadow-primary/30" onClick={handleProceedToQR} disabled={getFinalAmount() < 10}>
              <span>Pay ₹{getFinalAmount() || 0}</span><ArrowRight className="ml-2 h-5 w-5" />
            </Button>
            <p className="text-center text-xs text-muted-foreground">Minimum deposit: ₹10 • Instant credit to wallet</p>
          </div>
        )}
        {step === "qr" && (
        ...
        {step === "success" && (
          <div className="space-y-8 pt-12 animate-in fade-in zoom-in-95 duration-500">
            <div className="flex justify-center"><div className="relative"><div className="absolute inset-0 bg-green-500/20 rounded-full blur-2xl animate-pulse" /><div className="relative w-28 h-28 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center shadow-xl shadow-green-500/30"><Check className="h-14 w-14 text-white" strokeWidth={3} /></div></div></div>
            <div className="text-center space-y-2"><p className="text-3xl font-bold text-foreground">₹{getFinalAmount()} Added!</p><p className="text-muted-foreground">Your wallet has been credited successfully</p></div>
            <div className="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-500/20 rounded-2xl p-5">
              <div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">New Balance</p><p className="text-2xl font-bold text-foreground">₹{balance.toFixed(2)}</p></div><div className="w-12 h-12 rounded-xl bg-green-500/20 flex items-center justify-center"><Wallet className="h-6 w-6 text-green-500" /></div></div>
            </div>
