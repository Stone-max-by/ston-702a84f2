        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
            ...
            <div className="flex flex-wrap gap-1">
              <Badge variant="secondary" className="text-[9px] px-1.5 py-0">{productTypeLabels[product.type]}</Badge>
              <Badge variant="outline" className="text-[9px] px-1.5 py-0">{product.category}</Badge>
              {product.isNew && <Badge className="bg-green-500/20 text-green-400 border-0 text-[9px] px-1.5 py-0">NEW</Badge>}
              {product.tags?.slice(0, 2).map((tag) => (
                <Badge key={tag} variant="outline" className="text-[9px] px-1.5 py-0 bg-muted/50">{tag}</Badge>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-1 px-1.5 py-0.5 bg-white/5 rounded">
                <Download className="w-2.5 h-2.5" />{product.downloads}
              </span>
              <span className="flex items-center gap-1 px-1.5 py-0.5 bg-white/5 rounded">
              ...
              {product.rating && (
                <span className="flex items-center gap-1 px-1.5 py-0.5 bg-white/5 rounded">
                  <Star className="w-2.5 h-2.5 text-yellow-400 fill-yellow-400" />{product.rating.toFixed(1)}
                </span>
              )}
            </div>
            ...
            <div className="bg-background rounded-md p-2 border border-white/5">
              <p className="text-[10px] text-foreground/80 leading-relaxed line-clamp-3">{product.description}</p>
            </div>
            ...
            <div className="grid grid-cols-4 gap-1.5 text-[9px]">
              <div className="bg-background rounded-md p-1.5 border border-white/5 text-center">
                <HardDrive className="w-3 h-3 mx-auto mb-0.5 text-muted-foreground" />
                <span className="text-foreground">{product.sizeFormatted}</span>
              </div>
              <div className="bg-background rounded-md p-1.5 border border-white/5 text-center">
                <FileArchive className="w-3 h-3 mx-auto mb-0.5 text-muted-foreground" />
            ...
            <div className="flex items-center justify-between text-[10px] bg-background rounded-md p-2 border border-white/5">
              <div className="flex items-center gap-2 text-muted-foreground">
                {product.platform?.length > 0 && (
                  <span className="flex items-center gap-1"><Monitor className="w-3 h-3" />{product.platform.join(", ")}</span>
                )}
                {product.language?.length > 0 && (
                  <span className="flex items-center gap-1"><Globe className="w-3 h-3" />{product.language[0]}</span>
                )}
              </div>
          ...
          <div className="p-3 border-t border-white/5 bg-card">
            {isOwned ? (
              <Button onClick={handleAction} className="w-full h-9 text-xs font-semibold gap-1.5 bg-green-600 hover:bg-green-700">
                <CheckCircle className="w-3.5 h-3.5" />Download (Owned)
              </Button>
              ...
              <Button onClick={handleAction} className="w-full h-9 text-xs font-semibold gap-1.5 bg-amber-600 hover:bg-amber-700">
                <Play className="w-3.5 h-3.5" />Watch {product.adCreditsRequired} Ad{(product.adCreditsRequired || 1) > 1 ? 's' : ''}
              </Button>
            ) : (
              <Button onClick={handleAction} disabled={purchasing} className="w-full h-9 text-xs font-semibold gap-1.5">
